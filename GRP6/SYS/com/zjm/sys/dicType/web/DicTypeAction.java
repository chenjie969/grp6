...
public class DicTypeAction {
	 ...
	 * @return
	 */
	@RequestMapping(value="/selectDicTree")
	@ResponseBody
	public AjaxRes selectDicTree(@RequestBody C_dictype c_dictype){
		List<Map<String, Object>> mapList = new ArrayList<Map<String,Object>>();
		Map<String, Object> map = new HashMap<String, Object>();
		List<C_dictype> dicTypeList = dicTypeService.selectAllDicTypeList(" and (dicTypeID = \'"+c_dictype.getDicTypePID()+"\' or dicTypePID = \'"+c_dictype.getDicTypePID()+"\')");
		
		for (C_dictype dicType : dicTypeList) {
			map = new HashMap<String, Object>();
			if(c_dictype.getDicTypePID().equals(dicType.getDicTypeID())){
				map.put("open",true);
			}
			map.put("id", dicType.getDicTypeID());
			map.put("pId", dicType.getDicTypePID());
			map.put("name", dicType.getDicTypeName());
			mapList.add(map);
		}
		AjaxRes ar=new AjaxRes();
