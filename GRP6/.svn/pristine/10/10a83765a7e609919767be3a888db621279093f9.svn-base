(function($,z){
	 ...
	 */
	function initTable(){
		$("#table_hasVotedMeeting").bootstrapTable('destroy');
		$("#table_hasVotedMeeting").bootstrapTable({
			method: 'post',
						...
						{title:"操作",align: 'center',formatter:function(value,row,index){
							...
							events:{
								'click .btn_viewJurySuggest': function(e, value, row, index) {
									$.zjm_hasVoted.showMeetingDetail(row.meeting_ID);
								}
							}
						}
					 ],
			...
			url: "/project/jurySuggest/selectHasVotedMeetingPageTables",//这个接口需要处理bootstrap table传递的固定参数
//			ajaxOptions:{data:{"queryCondition.mod_uid":nodeid}},
	...
	function showMeetingDetail(meeting_ID){
		$("#page_jurySuggestIndex").load("/project/jurySuggest/selectMeetingWithJurySuggest",{"meeting_ID":meeting_ID,"showBtn":"view"},function(response,status,xhr){
			$("#proMeetingDetail").modal({keyboard:true});
			$("#btn_submitSuggest").hide();
			$(".btn_viewSuggestResult").click(function(){
				...
				$("#page_jurySuggestIndex2").load("/project/jurySuggest/showJurySuggestViewPage",param,function(response,status,xhr){
					$("#jurySuggestView").modal({keyboard:true});
					$("#edit_projectCode").text(projectCode);
					$("#edit_projectName").text(projectName);
					var zindex = parseInt($("#proMeetingDetail").css("z-index"));
					$("#jurySuggestView").css("z-index",zindex+50);
	...
	function openPage(pageNumber,pageSize){
		...
		$("#divHasVoted").load("/project/jurySuggest/showHasVotedDIVPage",{"pageNumber":pageNumber,"pageSize":pageSize},function(response,status,xhr){
			$(".btn_view").click(function() {
				var meeting_ID = $(this).attr("data-meetingID");	
				$.zjm_hasVoted.showMeetingDetail(meeting_ID);
			});
...
$(function () {
	...
	$.zjm_hasVoted.clickDivViewBtn();
	/**
	 * 点击列表按钮
	 */
	$("#btn_listView").click(function(){
		$.zjm_hasVoted.clickListViewBtn();
	});
