(function($,z){
	...
	/**初始化模态窗列表-单个项目的开票信息***/
	function initSglProBillTable(row){
...
//						{field:"billCode",title: '票据编号',align: 'center',sortable:"true",formatter: function (value, row, index) { return row.billCode;}},
						...
						{title: '操作',align: 'center',formatter:function(value,row,index){
								...
								btnArray.push('<button class="btn_singleProjectBill_view btn btn-xs btn-warning" href="javascript:void(0)" title="查看"><i class="icon-zoom-in bigger-120"></i></button>');
								if("edit"==$("#hidden_type").val()){
									btnArray.push('<button class="btn_singleProjectBill_edit btn btn-xs btn-info" href="javascript:void(0)" title="修改" data-toggle="modal" data-target="#edit_Page"><i class="icon-pencil bigger-120"></i></button>');
									btnArray.push('<button class="btn_singleProjectBill_del btn btn-xs btn-danger" href="javascript:void(0)" title="删除"><i class="icon-trash bigger-120"></i></button>');
								}
			...
			queryParams: function(params) {
				var queryCondition = {"applyDetail_ID":row.applyDetail_ID};
...
$(function () {
	...
	var row = new Object();
	row.applyDetail_ID = $("#hidden_applyDetailID").val();
	row.apply_ID = $("#hidden_applyID").val();
	
	$.zjm_sglProBillManage.initSglProBillTable(row);
	
	$("#btn_addSglProBillInfo").click(function(){
		$.zjm_sglProBillManage.addSingleProjectBill(row);
	});
