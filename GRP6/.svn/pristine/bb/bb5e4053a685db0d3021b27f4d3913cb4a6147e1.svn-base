(function($,z){
	$.zjm_executecase = {
			...
			openExecutecasePage:openExecutecasePage,//打开案件执行页面
			openExecutecaseEdit:openExecutecaseEdit, //打开案件执行编辑页面
			colseOneExecutecase:colseOneExecutecase,//关闭案件执行页面
			...
			insertOneExecutecase:insertOneExecutecase, //新增案件执行
	...
	function initData(){
		...
		//融投作为申请执行人显示资产划扣情况
		$("input[type=radio][name=executecaseType]").change(function(){
			if($(this).val() == '01') {
	...
	function initColumns1(){
		var columns = [
			...
			{field:'affiliate_roup',title:'涉及的集团及<br>所属公司名称',align:'center',sortable : "true",formatter: function (value, row, index) {return row.affiliateGroup;}},
			...
			{title: '操 作 ',align: 'center',formatter:function(value,row,index){
				return [
					'<button class="btn_executecase_edit btn btn-xs btn-info" title="修改" href="javascript:void(0)" data-toggle="modal" data-target="#edit_Page"><i class="icon-pencil bigger-120"></i></button>'
					].join('');
				},
				events:{
					'click .btn_executecase_edit': function(e, value, row, index) {
						$.zjm_executecase.openExecutecaseEdit(row);
					}
				}
			}
		];
	...
	function initExecutecase1Table(condition){
		...
		$('#executecase1-table').bootstrapTable({
			...
			url: "/project/executecase/selectExecutecasePageTables",//这个接口需要处理bootstrap table传递的固定参数
			...
			queryParams: function(params) {
				...
				$.extend(params,{"queryCondition":condition,"wheresql":" and executecase_type = '01' "});
				return params;
	...
	//案件执行页面
	function openExecutecasePage(){		
		window.parent.openMenu('openExecutecasePage','','案件执行','/project/executecase/openExecutecasePage',1);				
	}
	//案件执行编辑页面
	function openExecutecaseEdit(row){		
		window.parent.openMenu('openExecutecasePage','','编辑案件执行','/project/executecase/openExecutecasePage', '&executecaseId='+row.executecaseId ,1);				
	}
	//关闭案件执行页面
	function colseOneExecutecase(){
		window.parent.closeMenu('openExecutecasePage');
	}
	//案件执行
	function insertOneExecutecase(){
		if($("input[type=radio][name=isConSuit]:checked").val() == '1'){
			$("#assetSealId").val("");
			$("#assetSealName").val("");
		} else if($("input[type=radio][name=isConSuit]:checked").val() == '2'){
		...
		if($("#executecase_form").validationEngine("validate")){
			var queryContainer_form = $("#executecase_form");
			$.ajax({type:'POST',url:'/project/executecase/insertOrUpdateExecutecase',data:JSON.stringify(queryContainer_form.serializeJson()),contentType:'application/json; charset=UTF-8',dataType:'json',success:function(data) {
				if(data.obj==true){
			...
			tool.undisabled(".insertOneExecutecase");
		}
	}
	...
	function initSuitTable(condition) {
		...
		$('#table_lawSuit').bootstrapTable({
			...
			queryParams : function(params) {
				...
				//查询未关联过查封信息的诉讼列表
				var whereSql = " and projectLawsuit_ID not in(select CONCAT(law_suit_id) from pro_executecase where law_suit_id is not NULL or law_suit_id != '')";
	...
	function initAssetSealUpTable(condition) {
		...
		$('#table_assetSealUp').bootstrapTable({
			...
			queryParams : function(params) {
				...
				var whereSql = " and assetSealUp_ID not in(select CONCAT(asset_seal_id) from pro_executecase where asset_seal_id is not NULL or asset_seal_id != '')";
				//查询未关联过查封信息的诉讼列表
				$.extend(params,{"queryCondition":condition,"wheresql": whereSql});
				return params;
			},// 前端调用服务时，会默认传递上边提到的参数，如果需要添加自定义参数，可以自定义一个函数返回请求参数
...
$(function () {
	...
	function floaddata() {
		...
		$.zjm_executecase.initExecutecase1Table();
		$(".form-control-ztb").attr("placeholder",'输入项目名称,回车搜索');
	};
	
	//破产债权人列表
	$("#executecase1List").click(function(){
		$.zjm_executecase.initExecutecase1Table();
		$(".form-control-ztb").attr("placeholder",'输入项目名称,回车搜索');
	})
	//连带债务人列表
	$("#executecase2List").click(function(){
		$.zjm_executecase.initExecutecase2Table();
		$(".form-control-ztb").attr("placeholder",'输入项目名称,回车搜索');
	})
	//刷新列表
	...
	//打开执行案件页面
	$("#btn_openExecutecasePage").click(function(){
		$.zjm_executecase.openExecutecasePage();
	});
	//关闭案件执行
	$("#btn_colseOneExecutecase").click(function(){
		$.zjm_executecase.colseOneExecutecase();
	});
	//新增案件执行
	$("#btn_insertOneExecutecase").click(function(){
		$.zjm_executecase.insertOneExecutecase();
	});
	//关联诉讼信息选择
