					...
					<div class="widget-body">
						<div class="widget-main">
							<ul class="list-unstyled spaced2" style="text-align:center;">
								<li class="bigger-120">上会时间：<fmt:formatDate value="${mettingArrange.meetingDateTime }" pattern="yyyy-MM-dd HH:mm" type="both"/></li>
								<li class="bigger-120">会议室：${mettingArrange.meetingRoomName} </li>
							</ul>
							<hr>
							<div class="price blue">项目数量：<a href="javascript:void(0)" class="btn_view" data-meetingID="${mettingArrange.meeting_ID}">${mettingArrange.applyNum}</a></div>
						</div>
						<div style="height:44px;">
							<c:choose>
								<c:when test="${mettingArrange.meetingStatus eq'01'}">
									...
									<button class="btn btn-info width-50 pull-left btn_edit" data-meetingID="${mettingArrange.meeting_ID}"><i class="icon-pencil bigger-120"></i>修改</button>
									<%-- <button class="btn btn-warning width-25 pull-left btn_view" data-meetingID="${mettingArrange.meeting_ID}"><i class="icon-zoom-in bigger-120"></i>查看</button>
									<button class="btn btn-info width-25 pull-left btn_edit" data-meetingID="${mettingArrange.meeting_ID}"><i class="icon-pencil bigger-120"></i>修改</button>
									<button class="btn btn-success width-25 pull-left btn_print"><i class="icon-print bigger-120"></i>打印</button>
									<button class="btn btn-yellow width-25 pull-left btn_notice"><i class="icon-comments-alt bigger-120"></i>发送通知</button> --%>
								</c:when>
								<c:when test="${mettingArrange.meetingStatus eq'02'}">
