	...
	function initTable(){
		...
		$('#waitTask-table').bootstrapTable({
			...
			columns: [{field:'checked',checkbox:true,align: 'center',formatter: function (value, row, index) {return (index+1);}},
					...
					{field:"user_names",title: '当前办理人',align: 'center',formatter: function (value, row, index) { return row.transactorName;}},
					...
					{field:"userGroup_uuid",title: '操作',align: 'center',formatter:function(value,row,index){
						if(row.suspensionState==1){
							return [//'<button class="btn_modules_view btn btn-xs btn-warning" href="javascript:void(0)" onclick="window.parent.open(\'/gbpm/processInstance/selectProcessDiagram?procDefID='+row.procDefID+'&processInstanceID='+row.processInstanceID+'\')" ><i class="icon-zoom-in bigger-120"></i>查看</button>',
								...
								'<button class="btn_entrusted btn btn-xs btn-info" href="javascript:void(0)"><i class="icon-pencil bigger-120"></i>委托</button>'].join('');
						}
