(function($,z){
	...
	function addTableRow(){
		$(".table_refundInfo tbody").append('	<tr>'+	
										'	<td style="border:1px solid #ddd;"><!-- 费用类别 -->'+		
										'		<select class="validate[required] col-sm-12 costTypeList">'+
										' 		</select>'+
										'	</td>	'+
										'	<td style="border:1px solid #ddd;"><!-- 原收费金额（元） -->'+		
										'		<input type="text" style="width:80%;" class="tdinput">'+
										'	</td>	'+
										'	<td style="border:1px solid #ddd;"><!-- 退费金额（元）-->'+		
										'		<input type="text" style="width:80%;" class="tdinput">'+	
										'	</td>'+
										'	<td style="border:1px solid #ddd;"><!-- 退费后金额（元）-->'+		
										'		<input type="text" style="width:80%;" class="tdinput">'+
										'	</td>	'+
	...
	//删除一行收费信息
	$(".table_refundInfo tbody").on("click","button.delChargeInfoRow",function(){
		// 删除前先取行数，如果小于等于两行，删除后就只剩表头一行，就要立即新增一个空行
		var rowNum = $(".table_refundInfo").find("tr").length;
