...
alter table `pro_assetsealup` add `affiliateGroup` varchar(125) NOT NULL COMMENT '涉及公司名称';
...
CREATE TABLE `pro_bankruptcy` (
  ...
  `affiliate_group` varchar(125) DEFAULT NULL COMMENT '涉及集团及所属公司',
...
-- 修改任务事项URL
UPDATE `gbpm_dic_taskmanager` SET `taskUrl` = '/project/suggest/projectSuggetEditPage?suggestType=006' WHERE `taskManager_ID` =  '21983384efee44d1b7bc1a7e51c99270';
UPDATE `gbpm_dic_taskmanager` SET `taskUrl` = '/project/riskScheme/projectSuggetEditPage?suggestType=007' WHERE `taskManager_ID` =  '2429f2f0acab46898178d2e8daeb2022';

-- 添加索引
 alter table crm_relationmain add index AK_index_clientID (clientID);
 alter table pro_factpay add index FK_Relationship_applyID (applyID);

 -- 字段名修改
alter table pro_project CHANGE InterestMethodID interestMethodID varchar(32) DEFAULT NULL;
alter table pro_project CHANGE InterestMethodName interestMethodName varchar(20) DEFAULT NULL;
