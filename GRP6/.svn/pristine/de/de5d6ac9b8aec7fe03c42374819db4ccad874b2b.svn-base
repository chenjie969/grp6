(function($,z){
	$.zjm_projectTracking = {
			...
			editModule:editModule,//项目办理
			hightSelect:hightSelect,//高级查询
			overConfirm:overConfirm//逾期确认
	};
	...
	function initProjectTalble(columns){
		...
		$('#project-table').bootstrapTable({
			...
			queryParams: function(params) {
				 $.extend(params,{"wheresql":" and pp.delayEndDate >= now()"});
				return params;
	...
	function overConfirm(){
		...
		if(typeof(selectContent) == 'undefined') {  
			...
			$("#delModalLabel").html("逾期确认")
			$("#delMessage").html("项目是否已逾期？")
			tool.undisabled(".btn_submit");
			$(".btn_submit").click(function(){
				tool.disabled(".btn_submit");
				$.ajax({type:'POST',url:'/project/project/insertOneProjectOver',data:JSON.stringify({"project_ID":selectContent.project_ID}),contentType:'application/json; charset=UTF-8',dataType:'json',success:function(data) {
			        	if(data.obj==true){
...
$(function () {
	...
	$("#btn_overConfirm").click(function(){
		$.zjm_projectTracking.overConfirm();
	});
