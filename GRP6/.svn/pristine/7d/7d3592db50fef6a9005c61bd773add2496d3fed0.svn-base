...
$(function () {
	...
	$("#avatar").click(function () {
    	...
    	var zindex = parseInt($("#realizeOptGuarantyPage").css("z-index"));	//获取第一个模态窗的z-index。bootstrap的默认值，模态窗：z-index=1050，遮罩层：z-index=1040
...
function deleteOnePictureFile(projectFiles_ID,pathFile) {
	...
	$("#pictureFileDelModule").css("z-index",zindex+50);	//设置第二个模态窗的z-index，值比第一个模态窗大一些。
	$("#singleProjectBill_backdrop").nextAll(".modal-backdrop").css("z-index",zindex+40);	//获取第二个模态窗弹出时生成的遮罩层，并设置其z-index值，比第一个模态窗的z-index要大，比第二个模态窗的z-index要小
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.obj) {
                    ...
                    $("#"+projectFiles_ID+"DIV_realize").remove();
