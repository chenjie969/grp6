				...
				<form class="form-horizontal" role="form" id="nextTask_form">
					...
					<ul class="invest-history-list clearfix b0">
						<c:forEach items="${osRoleMap}" var="osRole">
							...
							<li class="i-h-l-item line-2 b0"><span class="fl w40 tr cti">${osRole.key}：</span> <span class="fl w30 tl cc"> <c:forEach items="${osRole.value}" var="flRolesUser" varStatus="status">
										<div style="float: left; height: 35px; line-height: 35px; margin: 0 10px;">
											<input type="checkbox" name="rolrUserUid" class="validate[minCheckbox[1]]" value="${flRolesUser.key }" id="flru_${status.index+1 }" /> <label for="flru_${status.index+1 }">${flRolesUser.value }</label>
										</div>
									</c:forEach>
							</span></li>
						</c:forEach>
