				...
				<div class="widget-box">
					...
					<div class="widget-body">
						<div class="widget-main">
							...
							<hr>
							<div class="price blue">项目数量：<a href="javascript:void(0)" class="btn_view btn_input" data-meetingID="${mettingArrange.meeting_ID}">${mettingArrange.applyNum}</a></div>
						</div>
						<div style="height:44px;">
							<c:choose>
								<c:when test="${isInput=='true'}">
								...
								<c:when test="${isInput=='false'}">
									...
									<button class="btn btn-info width-100 pull-left btn_input" data-meetingID="${mettingArrange.meeting_ID}"><i class="icon-pencil bigger-120"></i>录入评审会决议</button>
								</c:when>
							</c:choose>
						</div>
					</div>
