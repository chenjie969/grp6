...
public class MemoryWorkflowStore implements WorkflowStore {
    ...
    //zhongzk fixed，增加了stepName
    public Step createCurrentStep(long entryId,int  stepId,String stepName, String owner, Date startDate, Date dueDate, String status, long[] previousIds) {
        long id = globalStepId++;
        SimpleStep step = new SimpleStep(id, entryId, stepId,stepName, 0, owner, startDate, dueDate, null, status, previousIds, null);
    ...
    */
    ...
    //zhongzk 新增加 2012.04.22
    public List findOneCurrentSteps(long entryId,int stepID,String curuserid) {
        List currentSteps = (List) currentStepsCache.get(new Long(entryId));
