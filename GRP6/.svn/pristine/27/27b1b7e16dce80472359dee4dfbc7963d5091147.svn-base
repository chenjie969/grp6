(function($){
	...
	function isOtherStock(radioValue){
		if(radioValue == 1){
			...
			$.zjm_stockMessage.initStockMessageTable(clientID);
			$(".legalPersonInput").val("");
			$.ajax({
		...
		$("#btn_listOK").click(function(){	//弹出列表点击确定，将选中的企业名称填入以下几个位置：	
			...
			if(selectContent.length != 1){
				$("#pleaseSelectOne").modal({keyboard:true});
					...
					$.ZYequityPledge.initTable(selectContent[0].client_ID);
				}
			});
	...
	function addStock(){
		...
		$("#optGuarantyAdd_page").load("/optGuarantyAction/chooseProjectPage?t="+Math.random(),{"flag":"01"},function(){
				...
				if ($("#form_company").validationEngine("validate")) {
					...
					$.ajax({ 
						...
						success : function(data) {
							...
							$.ZYequityPledge.initTable(selectContent.client_ID);
						}
					});
				}	
			})
...
$(function(){
	...
	$.ZYequityPledge.initTable(client_ID);
	...
	$(".isCompanyStock").change(function(){
		...
		$.ZYequityPledge.isOtherStock(radioValue);
	})
