  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
								...
								<c:forEach items="${meeting.meetingApplyList}" var="meetingApply">
									<c:if test="${meetingApply.meetingSort == index }">
										<tr>
											...
											<c:choose>
												<c:when test="${! fn:contains(meetingApply.userIDList, user.user_uid)}">
													<td></td>
													<td>没有操作权限</td>
												</c:when>
												<c:otherwise>
													<c:choose>
														<c:when test="${not empty meetingApply.jurySuggest }">
															<td class="suggestResult">${meetingApply.jurySuggest.suggestResultName }</td>
															<td>
																<c:if test="${showBtn=='vote' }">
																	<a class="btn_applyVote" href="javascript:void(0)" data-jurySuggestID="${meetingApply.jurySuggest.jurySuggest_ID }">表决</a>
																</c:if>
																<c:if test="${showBtn=='view' }">
																	<a class="btn_viewSuggestResult" href="javascript:void(0)" data-jurySuggestID="${meetingApply.jurySuggest.jurySuggest_ID }">查看</a>
																</c:if>
														...
														<c:otherwise>
															<td class="suggestResult"></td>
															<td>
																<a class="btn_applyVote" href="javascript:void(0)" data-applyID="${meetingApply.proApply.apply_ID}">表决</a>
																<input type="hidden" value="${meetingApply.proApply.busiCode }">
																<input type="hidden" value="${meetingApply.proApply.projectName }">
															</td>
														</c:otherwise>
      ...
      <div class="modal-footer">
		<button type="button" class="btn btn-primary" id="btn_submitSuggest" > <i class='icon-ok bigger-110'></i>确认提交</button>
...
<script type="text/javascript">
	...
	$(".btn_proApply_view").click(function(){
		...
		$("#page_jurySuggestIndex2").load("/project/apply/projectApplyViewPage",{"apply_ID":apply_ID},function(response,status,xhr){
