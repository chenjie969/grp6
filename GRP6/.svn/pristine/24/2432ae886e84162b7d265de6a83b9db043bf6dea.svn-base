...
//				{title: '限办天数',align: 'center',formatter: function (value, row, index) { return row.transactorName;}},
			...
			url: "/gbpm/productInstance/selectProductMyBuildInstancePageTable",//这个接口需要处理bootstrap table传递的固定参数
			queryParamsType:'', //默认值为 'limit' ,在默认情况下 传给服务端的参数为：offset,limit,sort
			// 设置为 ''  在这种情况下传给服务器的参数为：pageSize,pageNumber
			queryParams: function(params) {
				$.extend(params,{"wheresql":" "});
				return params;
	...
	function initTableStop(nodeid,isChild){
		$('#postStop-table').bootstrapTable({
			...
			pageSize: 30,      //每页的记录行数（*）
			pageList: [30,50, 100, 200, 500],  //可供选择的每页的行数（*）
			url: "/gbpm/productInstance/selectProcessInstanceHistoryMyStartPageTable",//这个接口需要处理bootstrap table传递的固定参数
			queryParamsType:'', //默认值为 'limit' ,在默认情况下 传给服务端的参数为：offset,limit,sort
			// 设置为 ''  在这种情况下传给服务器的参数为：pageSize,pageNumber
			queryParams: function(params) {
