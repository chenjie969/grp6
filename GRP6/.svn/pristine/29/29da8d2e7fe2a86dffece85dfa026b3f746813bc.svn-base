(function($,z){
	$.zjm_transact = {
			...
			finisheScheme:finisheScheme,//审批完成
	...
	function initData(productInstanceID,nodeSort,classType){
		...
		var status = $("#status").val();//获取当前登陆人
		if(productNodeListLength==nodeSort){//如果当前处理节点是最后一步，则隐藏提交下一步按钮
			$(".btn_returnNext").hide();//隐藏提交下一步按钮
			$(".btn_finisheScheme").show();//显示审批完成按钮
		}else{
			$(".btn_returnNext").show();//显示提交下一步按钮
			$(".btn_finisheScheme").hide();//隐藏审批完成按钮
		}
		if(status == '审批通过'){
			$(".btn_finisheScheme").hide();//隐藏审批完成按钮
			$(".btn_returnBefore").hide();//隐藏退回按钮
			$(".btn_stopProcessPage").hide();//隐藏终止流程按钮
		};
	...
	function  getParamString (row){
		...
		paramString = '&entityID='+apply_ID
				    ...
				    +'&nodeID='+runNode_ID
				    +'&nodeNames='+encodeURI(encodeURI(nodeNames))
				    +'&taskID='+taskID
				    +'&taskName='+taskName
	 ...
	 * 跳转到否决项目页面
	 */
	function stopProcessPage(riskScheme_ID){
			$("#projectBeforeDeal_page").load("/project/riskScheme/stopProcessPage",{"riskScheme_ID":riskScheme_ID},function(response,status,xhr){
				...
				$(".btn_submit").click(function(){
					...
					if(queryContainer_form.validationEngine("validate")){
							$.ajax({type:'POST',url:'/project/riskScheme/stopProcess',data:JSON.stringify(queryContainer_form.serializeJson()),contentType:'application/json; charset=UTF-8',dataType:'json',success:function(data) {
						        	if(data.obj==1){
    ...
    function finisheScheme(){
    	 ...
    	 $.ajax({type:'POST',url:'/project/projectTracking/isAllFinish',data:JSON.stringify({"runNodeID":runNode_ID}),contentType:'application/json; charset=UTF-8',dataType:'json',success:function(data) {
    		 	if(data.obj==true){
					$("#common_del").modal({keyboard:true});
					$("#delModalLabel").text("提示");//标题;
					$("#delMessage").text("确认已完成审批？");//提示信息;
					tool.undisabled(".btn_submit");
					$(".btn_submit").click(function(){
						tool.disabled(".btn_submit");
						 var entityID = $("#apply_ID").val();//申请id
						$.ajax({type:'POST',url:'/project/riskScheme/finishRiskScheme',data:JSON.stringify({"riskScheme_ID":entityID}),contentType:'application/json; charset=UTF-8',dataType:'json',success:function(data) {
							if(data.obj==true){
								$("#common_del").modal("hide");
								window.parent.openMenu('riskrojectDeal'+entityID,'','流程办理','/project/projectTracking/riskProjectDeal','&riskScheme_ID='+entityID,1);
							}else{
								alert("操作失败！");
							}
						}
	    	 ...
	    	 $.ajax({type:'POST',url:'/project/projectTracking/isAllFinish',data:JSON.stringify({"runNodeID":runNode_ID}),contentType:'application/json; charset=UTF-8',dataType:'json',success:function(data) {
				if(data.obj==true){
						 var productID = $("#productID_01").val();//获取产品id
				    		 ...
				    		 var entityID = $("#apply_ID").val();//申请id
				    		 var entityType = $("#entityTypeID").val();//业务类型：申请：01 打包：02  风险处置：03
...
//	}
	...
	$(".btn_stopProcessPage").click(function(){
		var riskScheme_ID = $("#apply_ID").val();//获取业务id
		$.zjm_transact.stopProcessPage(riskScheme_ID);
	});
	//查看客户详情；
	...
	//审批完成
	$(".btn_finisheScheme").click(function(){
		$.zjm_transact.finisheScheme();		
	});
