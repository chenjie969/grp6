...
//			{field:'checked',checkbox:true,align:'center',formatter: function (value, row, index) {return ;}},
			...
			{field:'hostAreaName',title:'承办地区',align:'center',formatter: function (value, row, index) {return row.hostAreaName;}},
			...
			{field:'subBankName',title:'合作子机构（或个人）',align:'center',formatter: function (value, row, index) {return row.subBankName;}},
			...
			{field:'fundName',title:'资金方子机构',align:'center',formatter: function (value, row, index) {return row.fundName;}},
			{field:'loadBeginDate',title:'起始日期',align:'center',formatter: function (value, row, index) {return tool.parseDate(row.loadBeginDate,'');}},
			{field:'loadEndDate',title:'到期日期',align:'center',formatter: function (value, row, index) {return tool.parseDate(row.loadEndDate,'');}},
			{field:'overSum',title:'逾期金额（万元）',align:'center',formatter: function (value, row, index) {return row.overSum;}},
			{field:'overDesc',title:'逾期原因',align:'center',formatter: function (value, row, index) {return row.overDesc;}},
//			{field:'periodMonthDay',title:'期限',align:'center',formatter: function (value, row, index) {return row.periodMonthDay;}},
//			{field:'periodMonthDay',title:'超期天数',align:'center',formatter: function (value, row, index) {
//					var  date= new Date();
//					return (date.getFullYear()-row.loadEndDate)/(24*60*60*1000);
//				}
//			},
...
//				$.extend(params,{"wheresql":" and IFNULL(replaceFreeSum,0)= 0 and (" +
//						"	(delayEndDate < now()  and pp.finishDate is NULL) or " + //再报项目到期时间小于当前时间
//						"	(pp.isOver = 1 and (pp.finishDate is null or pp.finishDate > pp.overDate ) )"+ //项目未完结并且逾期确认，在项目完结之前逾期（项目完结时间大于逾期时间）
//						") "});
				$.extend(params,{"wheresql":" and IFNULL(pp.replaceFreeSum,0)= 0 and ( pp.isOver = 1  or (pp.finishDate is NULL and pp.delayEndDate<=now()) or pp.finishDate> pp.overDate) "});
				return params;
