...
<div class="modal fade bs-example-modal-sm" id="uploadfiles" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
    ...
    <div class="modal-dialog" style="z-index:999999">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary" id="pickfiles"><i class='icon-search bigger-110'></i>选择文件
                </button>
                <button type="button" class="btn btn-primary" id="upload"><i
                        class='icon-upload-alt bigger-110'></i>上传
                </button>
...
<%-- <span id="optGuaranty_ID" style="display: none;">${opt.optGuaranty_ID }</span> --%>
...
<script src="<%=path%>/project/opt/imgUpload/optUpload.js?v=<%=vardate%>"></script>
<%@ include file="/project/opt/imgUpload/delPicture.jsp" %>
<script>
	var form_baseTwo = $("#form_baseTwo");
	var args='<input type="hidden" name="optGuaranty_ID" id="optGuaranty_ID" value='+'${opt.optGuaranty_ID }'+'>';
	form_baseTwo.append(args);
</script>
