//注意:这是内部需要使用的函数，
//除 : ocxElement.codebase = "ntkoWebSign.cab#version=3,0,9,2";
...
function NtkoReserved_AddSecSignOcx(ControlID, ocxLeft, ocxTop){
	...
	try {
		...
		ocxElement.codebase = "ntkoWebSign.cab#version=3,0,9,2";
...
function NtkoReserved_RunSignHelper(ocxElement, UserName, FileName,
		PromptSelect, PrintMode, IsUseCertificate, IsLocked, IsCheckDocChange,
		IsShowUI, SignPass, SignType, IsAddComment, AdjustToHeight, SignIndex) {
	if ("object" != typeof (ocxElement))
		return;
	...
	case 1: {
		try {
			ocxElement.DoHandSign(PrintMode, IsUseCertificate, IsLocked,
					IsCheckDocChange, IsShowUI, SignPass, IsAddComment,
					AdjustToHeight);
		} catch (err) {
			alert("手写签名错误!");
			ocxElement.Close();
	...
	case 2: {
		try {
			ocxElement.DoSignFromEkey(SignPass, PrintMode, IsUseCertificate,
					IsLocked, IsCheckDocChange, IsShowUI, SignIndex,
					IsAddComment);
		} catch (err) {
			alert("加盖EKEY印章错误!");
			ocxElement.Close();
	...
	case 3: {
		try {
			ocxElement.DoKeyBoardComment(PrintMode, IsUseCertificate, IsLocked,
					IsCheckDocChange, IsShowUI, SignPass);
		} catch (err) {
			alert("添加安全键盘批注错误!");
			ocxElement.Close();
			ocxElement.removeNode();
