...
function markOtherInfo(){
	$.post(
		"/oa/loadOtherOaMarkInfo.action",
		{
			"cgsDocs.ueditformid" : $("#customToolBar").attr("data-uformid"),//备注 : 此字段是表单ID
			"cgsDocs.flowID" : $("#customToolBar").attr("data-flowid") //备注 : 此字段是流程ID
		},
		function(data){
			//console.log(data);
			//发文编号
			...
			if (TANGER_OCX_OBJ.ActiveDocument.BookMarks.Exists("qianFaRen")) {
				try {
					TANGER_OCX_OBJ.ActiveDocument.BookMarks.Item("qianFaRen").Range.Text = data.qianFaRen;
				} catch (e) {
			...
			if (TANGER_OCX_OBJ.ActiveDocument.BookMarks.Exists("faSongBuMen")) {
				try {
					TANGER_OCX_OBJ.ActiveDocument.BookMarks.Item("faSongBuMen").Range.Text = data.mainsenddepart;
				} catch (e) {
