...
//			cell.setCellStyle(style);
		...
		rownum = 0;// 如果rownum = 1，则去掉创建人、创建时间等副标题；如果rownum = 0， 则把标题也去掉
...
//			buildStyle(wb, src, sheet, 2, ret, "SUB_TITLE2");// 副标题2样式
			...
			buildStyle(wb, src, sheet, 0, ret, "TABLE_HEADER");// 表头样式
			buildStyle(wb, src, sheet, 1, ret, "STRING");// 字符串单元格样式
			buildStyle(wb, src, sheet, 2, ret, "INT");// 整数单元格样式
			buildStyle(wb, src, sheet, 3, ret, "D2");// 2位小数单元格样式
			buildStyle(wb, src, sheet, 4, ret, "D3");// 3位小数单元格样式
