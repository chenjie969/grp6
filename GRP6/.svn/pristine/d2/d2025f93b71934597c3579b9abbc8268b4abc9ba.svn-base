...
public class JDBCWorkflowStore implements WorkflowStore {
    ...
    //zhongzk 2012.04.22 增加步骤名称
    protected String stepStepName;
    ...
    public PropertySet getPropertySet(long entryId) {
        ...
        return PropertySetManager.getInstance("jdbc", args);//使用了propertySet专有技术，它的存储可以是数据库，XML，内存。
    }
    ...
    //zhongzk 2012.04.22，增加了wfStepName参数
    public Step createCurrentStep(long entryId, int wfStepId,String wfStepName, String owner, Date startDate, Date dueDate, String status, long[] previousIds) throws StoreException {
        Connection conn = null;

        try {
            conn = getConnection();

            //zhongzk fixed 增加了wfStepName
            long id = createCurrentStep(conn, entryId, wfStepId,wfStepName, owner, startDate, dueDate, status);
            addPreviousSteps(conn, id, previousIds);

            //zhongzk 2012.04.22在SimpleStep函数中增加参数wfStepName
            return new SimpleStep(id, entryId, wfStepId,wfStepName, 0, owner, startDate, dueDate, null, status, previousIds, null);
        } catch (SQLException e) {
    ...
    //创建流程实例,在流程实例表os_wfentry中插入一行数据
    public WorkflowEntry createEntry(String workflowName) throws StoreException {
        ...
        try {
            ...
            long id = getNextEntrySequence(conn);//从os_entryids表中取得下一个流程实例的ID
            stmt.setLong(1, id);
    ...
    public List findCurrentSteps(long entryId) throws StoreException {
        ...
        try {
            ...
            //zhongzk 增加了stepName 
            //zhongzk fixed 2015.01.14 处理动态分支时，只退回当前用户的当前步骤
            String sql = "SELECT " + stepId + ", " + stepStepId + ", " + stepActionId + ", " + stepOwner + ", " + stepStartDate + ", " + stepDueDate + ", " + stepFinishDate + ", " + stepStatus + ", " + stepCaller + ", " + stepStepName + " FROM " + currentTable + " WHERE " + stepEntryId + " = ?";
            /*
            String curUserID=String.valueOf(SystemSession.getUserSession().getUser_uid());
            System.out.println("test curuserid:"+curUserID);
            ...
            */
            ...
            while (rset.next()) {
                ...
                //zhongzk
                String stepName=rset.getString(10);
                ...
                //zhongzk fixed 2012.04.22 增加stepName参数
                SimpleStep step = new SimpleStep(id, entryId, stepId,stepName, actionId, owner, startDate, dueDate, finishDate, status, prevIds, caller);
    ...
    //zhongzk 2012.04.22，对上面的原方法findCurrentSteps进行了改造，以便支持 查找当前某一个步骤，最终实现只取某一个人某一个当前待办步骤上的所有动作
    public List findOneCurrentSteps(long entryId,int stepID,String curuserid)throws StoreException{
    	Connection conn = null;
        PreparedStatement stmt = null;
        ResultSet rset = null;
        PreparedStatement stmt2 = null;
        ...
        try {
            ...
            //zhongzk 增加了stepName
            String sql = "SELECT " + stepId + ", " + stepStepId + ", " + stepActionId + ", " + stepOwner + ", " + stepStartDate + ", " + stepDueDate + ", " + stepFinishDate + ", " + stepStatus + ", " + stepCaller + ", " + stepStepName + " FROM " + currentTable 
                       + " WHERE " + stepEntryId + " = ?"
                       + " and "+stepStepId + " = ?"
                       + " and OWNER"+ " = ?";
            ...
            //zhongzk 增加处理动态分支步骤名称相等时需要判断当前用户
            stmt.setString(3,curuserid );
    ...
    public List findHistorySteps(long entryId) throws StoreException {
        ...
        try {
            ...
            //zhongzk 2012.04.22增加stepName
            String sql = "SELECT " + stepId + ", " + stepStepId + ", " + stepActionId + ", " + stepOwner + ", " + stepStartDate + ", " + stepDueDate + ", " + stepFinishDate + ", " + stepStatus + ", " + stepCaller  + ", " + stepStepName + " FROM " + historyTable + " WHERE " + stepEntryId + " = ? ORDER BY " + stepId + " DESC";
    ...
    public void init(Map props) throws StoreException {
        ...
        //zhongzk 增加步骤名称
        stepStepName=getInitProperty(props, "step.stepName", "stepName");
      //  System.out.println("stepName:"+stepStepName);
    ...
    public Step markFinished(Step step, int actionId, Date finishDate, String status, String caller) throws StoreException {
        ...
        try {
           ...
           // stmt.setTimestamp(3, new Timestamp(finishDate.getTime()));
            ...
            String sFinishDateTime = df1.format(finishDate.getTime());
            Timestamp finishDateTime = Timestamp.valueOf(sFinishDateTime);
            stmt.setTimestamp(3, finishDateTime);
    ...
    public void moveToHistory(Step step) throws StoreException {
        ...
        try {
            ...
            String sql = "INSERT INTO " + historyTable + " (" + stepId + ',' + stepEntryId + ", " + stepStepId + ", " + stepActionId + ", " + stepOwner + ", " + stepStartDate + ", " + stepFinishDate + ", " + stepStatus + ", " + stepCaller+ ", " +stepStepName+ ") VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
           ...
           // System.out.println("sql:"+sql);
            ...
            //stmt.setTimestamp(6, new Timestamp(step.getStartDate().getTime()));zhongzk fixed
            SimpleDateFormat df1 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            String sStartDateTime = df1.format(step.getStartDate().getTime());
            Timestamp startDateTime = Timestamp.valueOf(sStartDateTime);
            stmt.setTimestamp(6, startDateTime);
            

            if (step.getFinishDate() != null) {
                //stmt.setTimestamp(7, new Timestamp(step.getFinishDate().getTime())); zhongzk fixed
            	SimpleDateFormat df2 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                String sFinishDateTime= df1.format(step.getFinishDate().getTime());
                Timestamp finishDateTime = Timestamp.valueOf(sFinishDateTime);
                stmt.setTimestamp(7, finishDateTime);
           ...
           // stmt.setString(9, step.getCaller()); zhongzk fixed
            stmt.setString(9, step.getOwner());
            
            
            
            //zhongzk 增加步骤名称
            stmt.setString(10,step.getStepName());
    ...
    protected long getNextEntrySequence(Connection c) throws SQLException {
        ...
        try {
          ...
          //  System.out.println("取得流程实例的下一个ID的sql:"+entrySequence);
            rset = stmt.executeQuery();
            rset.next();
    ...
    //zhongzk 2012.04.22增加wfStepName叁数
    protected long createCurrentStep(Connection conn, long entryId, int wfStepId,String wfStepName, String owner, Date startDate, Date dueDate, String status) throws SQLException {
        String sql = "INSERT INTO " + currentTable + " (" + stepId + ',' + stepEntryId + ", " + stepStepId+", "+stepStepName + ", " + stepActionId + ", " + stepOwner + ", " + stepStartDate + ", " + stepDueDate + ", " + stepFinishDate + ", " + stepStatus + ", " + stepCaller + " ) VALUES (?, ?, ?,?, null, ?, ?, ?, null, ?, null)";
       // String sql = "INSERT INTO " + currentTable + " (" + stepId + ',' + stepEntryId + ", " + stepStepId+", "+stepStepName + ", " + stepActionId + ", " + stepOwner + ", " + stepStartDate + ", " + stepDueDate + ", " + stepFinishDate + ", " + stepStatus + ", " + stepCaller + " ) VALUES (?, ?, ?,?, null, null, NOW(),   null, null,  null, null)";
        ...
        //zhongzk 增加
        stmt.setString(4,wfStepName);
        
       stmt.setString(5, owner);
       
       //stmt.setTimestamp(6, new Timestamp(startDate.getTime()));      
       SimpleDateFormat df1 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
       String sStartDateTime = df1.format(startDate.getTime());
       Timestamp startDateTime = Timestamp.valueOf(sStartDateTime);
       stmt.setTimestamp(6, startDateTime);
        ...
        if (dueDate != null) {        	
            //stmt.setTimestamp(7, new Timestamp(dueDate.getTime()));
            SimpleDateFormat df2 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            String sDueDateTime = df2.format(dueDate.getTime());
            Timestamp dueDateTime = Timestamp.valueOf(sDueDateTime);
            stmt.setTimestamp(7,dueDateTime);
