(function($,z){
	$.zjm_yesSignAnnounce = {
			...
			viewAnnounce:viewAnnounce,//查看消息详情
			delAnnounce:delAnnounce,//删除
	...
	function initTable(messageTypeId,treeLevel,data){
		$("#yesSign_table").bootstrapTable('destroy');
		$('#yesSign_table').bootstrapTable({
			method: 'get',
			columns: [
	...
	function delOneRelationAnnounce(row){
		...
		$.zjm_yesSignAnnounce.delAnnounce(ids);
	}
	/**删除记录**/
		...
		$("#info_page").load("/oa/announce/yesSignAnnounceAdQuery",{},function(response,status,xhr){
			...
			$(".btn_submit").click(function(){
				...
				if(queryContainer_form.validationEngine("validate")){
					...
					initTable($("#currentTreeId").val(),$("#treeLevel").val(),$("#query_annouce_form").serializeJson());	//传入高级查询条件后重新请求列表
				}else{
					tool.undisabled(".btn_submit");
				}
			});
...
$(function () {
	...
	$("#btn_agreeToAccept3").click(function(){
		var selectContent = $("#yesSign_table").bootstrapTable('getSelections');
	...
	$("#btn_advancedQuery").click(function(){
		$.zjm_yesSignAnnounce.advancedQuery();
	});
	$("#btn_refresh").click(function(){
		$.zjm_yesSignAnnounce.initTable($("#currentTreeId").val(),$("#treeLevel").val());
	});
});
