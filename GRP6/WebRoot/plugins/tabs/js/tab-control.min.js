(function() {
    ...
    //zhongzk 点击左边栏目菜单项后弹出一人tab，并且显示一个页面内容
    var p = function(A, z, B, C) {
        ...
        if (D > -1) {
            ...
            if (z) {
                ...
                jQuery(a).append('<iframe src="' + B + '" scrolling="auto" frameborder="0" allowfullscreen="true" reload="' + C + '"></iframe>');
            }
        }
        i();
    };
    ...
    var removeLeft = function(z) {
        ...
        for (var y = 0; x[y]; y++) {
        	...
        	w(x[y].getAttribute("index"));
        }
        i();
    };
    var removeRight = function(z) {
        var x = jQuery(t).find("li");
        var B = jQuery(r).find("li");
        var A = jQuery(a).find("iframe");
        var count;
    ...
    TabControlAppend = p;
    TabControlRemove = w;
    TabControlRemoveLeft = removeLeft;
    TabControlRemoveRight = removeRight;
    TabControlRemoveOther = removeOther;
    TabControlRemoveAll = removeAll;
    i();
})();
