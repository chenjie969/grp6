(function($,z){
	...
	/**初始化列表***/
	function initTable(){
		$("#entrustOther-table").bootstrapTable('destroy');
		$('#entrustOther-table').bootstrapTable({
			method: 'get',
				...
				{field:"userGroup_uuid",title: '操作',align: 'center',formatter:function(value,row,index){
						...
						events:{
							'click .btn_modules_view': function(e, value, row, index) {
								$.zjm_entrustOther.viewUserGroup(row);
							},
							'click .btn_modules_edit': function(e, value, row, index) {
								$.zjm_entrustOther.editUserGroup(row);
							},
							'click .btn_modules_del': function(e, value, row, index) {
								$.zjm_entrustOther.delUserGroup(row);
							}
						}
					}],
			detailView: true,
			...
			url: "/gbpm/processInstance/selectProcessTaskEntrustedPageTable",//这个接口需要处理bootstrap table传递的固定参数
