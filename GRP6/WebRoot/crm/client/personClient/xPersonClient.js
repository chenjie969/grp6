(function($,z){
	$.zjm_persionClient = {
			...
			xsynchro:xsynchro,//同步
			initProjNameTable:initProjNameTable,//初始化该客户下申请
	...
	function initColumns(condition){
		...
		$.ajax({type:'POST',url:'/selectOneListSetInfo',data:JSON.stringify({"listBH":'003'}),contentType:'application/json; charset=UTF-8',dataType:'json',success:function(data) {
	...
	function addOnePersonClient(){
		$("#addXPersonClientModal").modal({keyboard:true});
		zjm.init();
		$.ajax({type:'POST',
		...
		/**注册编辑验证引擎*/
		zjm.validata({formId:"add_xPersonClientForm"});
		/**提交*/
		tool.undisabled(".btn_submit");
		$(".btn_submit").click(function(){
			tool.disabled(".btn_submit");
			if($("#add_xPersonClientForm").validationEngine("validate")){
					var queryContainer_form = $("#add_xPersonClientForm");
					if(zjm.ajaxValidata("add_personNum","/selectPersonNameIsExist",JSON.stringify(queryContainer_form.serializeJson()),"申请人身份证号已存在！")){
						$.ajax({type:'POST',url:'/insertOneXPersonClientInfo',data:JSON.stringify(queryContainer_form.serializeJson()),contentType:'application/json; charset=UTF-8',dataType:'json',success:function(data) {
							if(data.obj==1){
									$("#addXPersonClientModal").modal("hide");
									$(".ztb_add").val("");
		...
		$("#addXPersonClientModal").on("hidden.bs.modal", function (e) {//解除事件绑定
	...
	function hightSelect(){
		$("#hightSelectXPersonClient").modal({keyboard:true});
		zjm.init();
		//获取部门下拉树;
		...
		/**注册编辑验证引擎*/
		zjm.validata({formId:"hightSelect_xPersonClientForm"});
		/**提交*/
		tool.undisabled(".btn_submit");
		
		$(".btn_submit").click(function(){
			tool.disabled(".btn_submit");
			if($("#hightSelect_xPersonClientForm").validationEngine("validate")){
					var queryContainer_form = $("#hightSelect_xPersonClientForm");
					var condition = queryContainer_form.serializeJson();
					$("#hightSelectXPersonClient").modal("hide");
					$(".ztb_hightSelect").val("");
		...
		$("#hightSelectXPersonClient").on("hidden.bs.modal", function (e) {//解除事件绑定
			 $(".btn_submit").unbind("click");
		});
	}
	...
	function listSet(){
		...
		$("#listBH").val("003");
		zjm.dataColumnsVal("/selectChoosableColumnsJSON",{"listBH":'003'},"ztb_choosable");
		zjm.dataColumnsVal("/selectSelectedColumnsJSON",{"listBH":'003'},"ztb_selected");
	        	...
	        	$.map($('#personClient-table').bootstrapTable('getSelections'), function(row) {	        		
		     		...
		     		$(".btn_submit").click(function(){	 
		    	        ...
		    	        if(typeof(selectContent2) == 'undefined') {  
		    	        	...
		    	        	$("#synchroPersonClientmodule").on("hidden.bs.modal", function (e) {// 解除事件绑定
		    	        		$(".btn_submit").unbind("click");
		    	        	});
		     		});
	 ...
	 * 初始化申请名称列表
	 */
	function initProjNameTable(guid){
...
$(function () {
	 ...
	 * 同步
	 */
	$("#xbtn_synchro").click(function(){
		$.zjm_persionClient.xsynchro();
	});
