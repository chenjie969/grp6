...
(function($,z){
	$.zjm_documentManager = {
			...
			documentView:documentView,// 查看文档 ---已生成文旦
	...
	function initColumns(condition){
		var columns = [
			...
			{field:'uploadDateTime',title:'上传时间',align:'center',formatter: function (value, row, index) {return tool.parseDate(row.uploadDateTime);}},
			{title: '操作',align: 'center',formatter:function(value,row,index){
				if(row.extend == 'docx' || row.extend == 'doc' || row.extend == 'pdf' || row.extend == 'xlsx' || row.extend == 'xls' || row.extend == 'ppt' || row.extend == 'pptx'){
					return ['<button title="查看模板" class="btn btn-xs btn-warning" onclick="window.parent.open(\'/sys/documentPreview/selectDocumentViewPage?domhref='+row.mouldPath+'&domextend='+row.extend+'\')"><i class="icon-zoom-in bigger-120"></i></button>',
						'<button title="下载模板" class="btn btn-xs btn-warning" onclick="window.parent.open(\'/crm/filesUpload/selectOneFilesDownload?fileName='+row.oldMouldName+'&filePath='+row.mouldPath+'\')"><i class="icon-download-alt bigger-120"></i></button>',
		...
		var columns2 = [
			...
			{field:'fileSize',title:'文件大小',sortable:"true",align:'center',formatter: function (value, row, index) {return (row.fileSize/1024).toFixed(2)+'kb';}},
			{field:'uploadDateTime',title:'生成时间',align:'center',formatter: function (value, row, index) {return tool.parseDate(row.uploadDateTime);}},
			...
			{title: '操 作 ',align: 'center',formatter:function(value,row,index){
				return [
					...
					'<button title="下载文档" class="btn btn-xs btn-warning" onclick="window.parent.open(\'/crm/filesUpload/selectOneFilesDownload?fileName='+row.sourceFileName+'&filePath='+row.pathFile+'\')"><i class="icon-download-alt bigger-120"></i></button>',
	...
	function generateDocument(row){
		...
		$.ajax({
			...
			data:JSON.stringify({"sourceFileName":row.oldMouldName,"fileType":row.mouldTypeName,"pathFile":row.mouldPath,
...
$(function () {
	...
	function floaddata() {
		...
		$.zjm_documentManager.relieveTable(); //默认加载已解除列表
	...
	$("#relieveList").click(function(){ // relieveList
		$.zjm_documentManager.relieveTable();
	});
	...
	$("#disposeList").click(function(){ //disposeList
		$.zjm_documentManager.disposeTable();
	});
