...
$.zjm_upload_os = {
	...
	function initUpload(param){
		...
		$.zjm_upload_os.activateUploadBuild(defParam);
	}
	...
	function initTable(defParam){
		...
		$("#"+defParam.fileList).bootstrapTable({
			...
			columns: [{title: '序号',align: 'center',formatter: function (value, row, index) {return index+1;}}, 
				...
				{field:"uploadDateTime",title: '上传时间',align: 'center',sortable:"true",formatter: function (value, row, index) { return tool.dateFormat(row.uploadDateTime,"yyyy-MM-dd");}},
				...
				{title: '操作',align: 'center',formatter:function(value,row,index){
					return ['<i class="danger icon-trash bigger-120 btn_osopfile_del"></i>'].join('');
				},
				events:{
					'click .btn_osopfile_del': function(e, value, row, index) {
						$.zjm_upload_os.osdelFile(row,defParam,defParam.uploadParam.fileOneType);
					}
				}
				}],
				detailView: false,
