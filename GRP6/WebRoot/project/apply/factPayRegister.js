(function($,z){
	$.zjm_projectPayDelay = {
		...
		rules : rules,//页面验证规则
		initOverTable:initOverTable, //初始化-逾期-列表
...
//				 var queryCondition ={"project_ID":$("#project_ID").val()}; 
	...
	function initOverTable(){
		$('#projectOver_table').bootstrapTable('destroy');
		$('#projectOver_table').bootstrapTable({
			method: 'post',
			columns: [	
					{title: '序号',align: 'center',formatter: function (value, row, index) {return index+1;}}, 	
					{field:"overSum",title: '逾期金额',align: 'center',formatter: function (value, row, index) { return row.overSum}},
					{field:"overDate",title: '上报日期',align: 'center',formatter: function (value, row, index) { return tool.parseDate(row.overDate)}},
					{field:"overFactDate",title: '逾期日期',align: 'center',formatter: function (value, row, index) { return tool.parseDate(row.overFactDate)}},
					{field:"overDesc",title: '逾期原因',align: 'center',formatter: function (value, row, index) { return row.overDesc}},
					],
			detailView: false,
			detailFormatter:function(index, row){
			    var html = [];
			        html.push('<p><b>逾期金额：</b> ' + row.overSum + '</p>');
			    return html;
			},
			toolbar: '#toolbar',    //工具按钮用哪个容器
			striped: true,      //是否显示行间隔色
...
$(function () {
		...
		$.zjm_projectPayDelay.initPayTable();
		$.zjm_projectPayDelay.initDelayTable();
		$.zjm_projectPayDelay.initOverTable();
		//还款登记
