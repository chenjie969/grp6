...
//   }
	function projectAfterAdd(){
		/**注册编辑验证引擎*/
		zjm.validata({
			formId:"projectAfter_form"
		});
		tool.disabled(".btn_submit");
		if($("#projectAfter_form").validationEngine("validate")){
			var queryContainer_form = $("#projectAfter_form");
			$.ajax({type:'POST',url:'/project/project/insertProjectInfoAfter',data:JSON.stringify(queryContainer_form.serializeJson()),contentType:'application/json; charset=UTF-8',dataType:'json',success:function(data) {
				if(data.obj){
					alert("保存成功！");
