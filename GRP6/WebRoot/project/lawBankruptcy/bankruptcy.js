		...
		$("#btn_suitListOK").click(
				function() { // 弹出列表点击确定，将选中的企业名称填入以下几个位置：
					var selectContent = $("#table_lawSuit")
							.bootstrapTable('getSelections');
					var isSuitMaxSum = $("input[name='isSuitMaxSum']:checked").val(); 
					if (selectContent.length == 0 &&isSuitMaxSum == 1) {
						$("#pleaseSelectOne").modal({
							keyboard : true
						});
					} else if (selectContent.length != 1 && isSuitMaxSum == 0) {
						$("#pleaseSelectOne").modal({
							keyboard : true
						});
					} else {
		...
		$("#btn_assetListOK").click(
				function() { // 弹出列表点击确定，将选中的企业名称填入以下几个位置：
					var selectContent = $("#table_assetSealUp")
							.bootstrapTable('getSelections');
					var isAssetMaxSum = $("input[name='isAssetMaxSum']:checked").val(); 
					if (selectContent.length == 0 && isAssetMaxSum == 1) {
						$("#pleaseSelectOne").modal({
							keyboard : true
						});
					} else if (selectContent.length != 1 && isAssetMaxSum == 0) {
						$("#pleaseSelectOne").modal({
							keyboard : true
						});
					} else {
