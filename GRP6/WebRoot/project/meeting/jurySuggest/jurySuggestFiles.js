(function ($, z) {
    ...
    function initUpload(param) {
        ...
        $.zjm_jurySuggestFiles.activateUploadBuild(defParam);
    }
        ...
        uploader = new plupload.Uploader({
            ...
            init: {
                ...
                UploadComplete: function (up, files) { //上传成功后，在页面进行显示
                    ...
                    $.get("/project/jurySuggest/getAttachments?entityID=" + $("#jurySuggest_ID").val(), function (data) {
                        ...
                        if (data.obj) {
                            $.each(data.obj, function (k, v) {
                                var aHref = ["<div id='" + v.projectFiles_ID + "DIV'><a href='" + v.pathFile + "' target='_blank'>" + v.sourceFileName + "</a>",
                                    '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;',
//                                    "<a href='javascript:void(0)' onclick='$.zjm_jurySuggestFiles.deleteOnePictureFile(this.id,this.name)' name='" + v.pathFile + "' id='" + v.projectFiles_ID + "'>" + '删除' + "</a>",
//                                    '<button type="button" title="删除" class="btn btn-xs btn-danger" href="javascript:void(0)" onclick="$.zjm_jurySuggestFiles.deleteOnePictureFile(this.id,this.name)" name="'+v.pathFile+'" id="'+v.projectFiles_ID'+"><i class="icon-trash bigger-120"></i></button>',
                                    '<button type="button" title="删除" class="btn btn-xs btn-danger" href="javascript:void(0)" onclick="$.zjm_jurySuggestFiles.deleteOnePictureFile(this.id,this.name)" name="'+v.pathFile+'" id="'+v.projectFiles_ID+'"><i class="icon-trash bigger-120"></i></button>',
                                    "<br/></div>"].join('');
    ...
    function filesUpdate(jurySuggest_ID){
		var defParam = {
	            ...
	            "uploadParam": {
	    				...
	    				"fileTwoType":"01",//附件上传入口分类	01:评委表决附件
	    				"clientID":"",//客户id
	    				"projectID":"",//项目id
	    				"fileFlag":"projFiles",
	    				"entityID":jurySuggest_ID,
	    				"uuid":""//文件uuid
	            },//上传附加参数
		...
		var zindex = parseInt($("#jurySuggestEdit").css("z-index"));	//获取第一个模态窗的z-index。bootstrap的默认值，模态窗：z-index=1050，遮罩层：z-index=1040
    	
    	$("#uploadfiles").modal({keyboard: true});
		$("#uploadfiles").css("z-index",zindex+50);	//设置第二个模态窗的z-index，值比第一个模态窗大一些。
		$(".modal-backdrop:eq(2)").css("z-index",zindex+40);	//获取第二个模态窗弹出时生成的遮罩层，并设置其z-index值，比第一个模态窗的z-index要大，比第二个模态窗的z-index要小
		
        
        $.zjm_jurySuggestFiles.initUpload(defParam);
