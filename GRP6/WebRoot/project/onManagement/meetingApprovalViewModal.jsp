					...
					<div class="row">
						<div class="">
							...
							<div class="form-group">
								...
								<label class="col-sm-9 light-grey"> 
									<c:forEach
										items="${meeting.packageProNameList}" varStatus="pac"
										var="pacNames">
										<c:choose>
							...
							<div class="form-group">
								...
								<label class="col-sm-9 light-grey">
									${meeting.updateUserName} </label>
							</div>
							<div class="space-4"></div>
							<div class="form-group">
								<label class="col-sm-3 control-label no-padding-right">申请状态：</label>
								<label class="col-sm-9 light-grey"> <c:choose>
										<c:when test="${meeting.meetingStatus eq '01'}">待审核</c:when>
										<c:when test="${meeting.meetingStatus eq '02'}">已同意</c:when>
										<c:when test="${meeting.meetingStatus eq '03'}">被拒绝</c:when>
...
function projectsMethod(pro) {
    ...
    $("#proEvalModal").load("/project/apply/projectApplyViewPage?apply_ID="+apply_ID,function(response,status,xhr){
    	var zindex = parseInt($("#view_meeting").css("z-index"));	//获取第一个模态窗的z-index。bootstrap的默认值，模态窗：z-index=1050，遮罩层：z-index=1040
