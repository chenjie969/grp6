...
<div class="modal fade" id="costDelayToPre" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-backdrop="static" data-keyboard="false" style="z-index:9999">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title">缓收转预收</h4>
      </div>
      <div class="modal-body">
		<form class="form-horizontal" role="form" id="form_costDelayToPre">
			<input type="hidden" name="apply_ID" value="${costDelay.apply_ID }">
			<input type="hidden" name="costDelay_ID" value="${costDelay.costDelay_ID }">
			<input type="hidden" name="applyDetail_ID" value="${costDelay.applyDetail_ID }">
			<input type="hidden" name="costStandard_ID" value="${costDelay.costStandard_ID }">
			
			<div class="form-group">
				<label class="col-sm-3 control-label no-padding-right" for="form-field-1">放款机构</label>
				<label class="col-sm-8" for="form-field-1">${costDelay.bankName } </label>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label no-padding-right" for="form-field-1">业务品种:</label>
				<label class="col-sm-8" for="form-field-1">${costDelay.busiTypeName } </label>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label no-padding-right" for="form-field-1">担保金额:</label>
				<label class="col-sm-8" for="form-field-1"><fmt:formatNumber value="${costDelay.agreeSum }" pattern="###,###.######"/> 万元</label>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label no-padding-right" for="form-field-1">费用类型:</label>
				<label class="col-sm-8" for="form-field-1">${costDelay.costStandard.costTypeName } </label>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label no-padding-right" for="form-field-1">费率:</label>
				<label class="col-sm-8" for="form-field-1">${costDelay.costStandard.costRate }&nbsp;${costDelay.costStandard.costUnit } </label>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label no-padding-right" for="form-field-1">缓收费用： </label>
				<label class="col-sm-8" for="form-field-1"><fmt:formatNumber value="${costDelay.delayCostSum }" pattern="###,###.##"/>元</label>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label no-padding-right" for="form-field-1">预收费用： </label>
				<label class="col-sm-8" for="form-field-1"><fmt:formatNumber value="${costDelay.delayCostSum }" pattern="###,###.##"/>元</label>
				<input type="hidden" name="preCostSum" value="${costDelay.delayCostSum }">
			</div>
			<div class="form-group">
