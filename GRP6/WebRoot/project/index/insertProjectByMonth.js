...
//			{field:'checked',checkbox:true,align:'center',formatter: function (value, row, index) {return ;}},
			...
			{field:'guarantyOrgName',title:'承保机构',align:'center',formatter: function (value, row, index) {return row.guarantyOrgName;}},
			{field:'hostAreaName',title:'地区',align:'center',formatter: function (value, row, index) {return row.hostAreaName;}},
			...
			{field:'bankName',title:'合作放款机构',align:'center',formatter: function (value, row, index) {return row.bankName;}},
			{field:'fundChinese',title:'资金方名称',align:'center',formatter: function (value, row, index) {return row.fundChinese;}},
			{field:'fundName',title:'资金方子机构（或个人姓名）',align:'center',formatter: function (value, row, index) {return row.fundName;}},
			...
			{field:'limitType',title:'期限种类',align:'center',formatter: function (value, row, index) {
//				if(row.periodMonth>0 && row.periodMonth<=6){return 1;}
//				else if(row.periodMonth>6 &&row.periodMonth<=12){return 2;}
//				else if(row.periodMonth>12 &&row.periodMonth<=36){return 3;}
//				else if(row.periodMonth>36 &&row.periodMonth<=60){return 4;}
//				else if(row.periodMonth>60){return 5;}
				return row.limitType;
				}
			},
			{field:'bankRate',title:'贷款利率（%）',align:'center',formatter: function (value, row, index) {return row.bankRate;}},
			{field:'bzScale',title:'保证金比例（%）',align:'center',formatter: function (value, row, index) {return row.bzScale;}},
			{field:'guarantyRate',title:'担保年费率（%）',align:'center',formatter: function (value, row, index) {return row.guarantyRate;}},
			{field:'serviceRate',title:'服务年费率（%）',align:'center',formatter: function (value, row, index) {return row.serviceRate;}},
			{field:'reviewRate',title:'评审费率（%）',align:'center',formatter: function (value, row, index) {return row.reviewRate;}},
			{field:'dcontractCode',title:'委托担保（委贷）合同号',align:'center',formatter: function (value, row, index) {return row.dcontractCode;}},
			{field:'jcontractCode',title:'借款合同号',align:'center',formatter: function (value, row, index) {return row.jcontractCode;}},
			{field:'bcontractCode',title:'保证合同号',align:'center',formatter: function (value, row, index) {return row.bcontractCode;}},
			{field:'isNewClient',title:'是否新户',align:'center',formatter: function (value, row, index) {return row.isNewClient==0?'否':'是';}},
...
//				$.extend(params,{"wheresql":" and YEAR(loadBeginDate) = YEAR(NOW()) AND MONTH(loadBeginDate) = MONTH(NOW()) "});
				$.extend(params,{"wheresql":" and YEAR(reportDate) = YEAR(NOW()) AND MONTH(reportDate) = MONTH(NOW()) "});
				return params;
