...
//			{field:'checked',checkbox:true,align:'center',formatter: function (value, row, index) {return ;}},
			...
			{field:'replaceOverDate',title:'代偿日期',align:'center',formatter: function (value, row, index) {return tool.parseDate(row.replaceOverDate,'');}},
...
//				$.extend(params,{"wheresql":" and replaceFreeSum > 0 and (	(delayEndDate < now()  and pp.finishDate is NULL) or 	(pp.isOver = 1 and (pp.finishDate is null or pp.finishDate > pp.overDate ) ))"});
				$.extend(params,{"wheresql":" and replaceFreeSum > 0  AND ( pp.isOver = 1  or (pp.finishDate is NULL and pp.delayEndDate<=now()) or pp.finishDate > pp.overDate)"});
				return params;
