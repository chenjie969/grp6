...
<div class="modal fade" id="updateCostMust" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel">修改应收</h4>
      </div>
      <div class="modal-body">
      		<form class="form-horizontal row" role="form" id="updateCostMust_form">
				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						<input type="hidden" id="costName" name="costName" class="costName"> 
						<select class="col-sm-4 validate[required] costTypeID" id="costTypeID" name="costTypeID">
							<c:forEach var="costType" items="${costTypeList}" varStatus="status">
								<option value="${costType.dicTypeID}" <c:if test="${costMust.costTypeID eq costType.dicTypeID}">selected="selected"</c:if> >${costType.dicTypeName}</option>
							</c:forEach>
						</select>
					</div> 
				...
				<div class="form-group ">
					...
					<div class="col-sm-9">
						<input type="text" name="costRate" onkeyup="calculationFee(this.value);" value="${costMust.costRate }" class="col-sm-4 validate[required] costRate" /> 
					</div>
				</div>
				...
				<div class="form-group ">
					...
					<div class="col-sm-9">
						<select class="col-sm-4 validate[required] costUnit" id="costUnit" name="costUnit">
							<option value="%" <c:if test="${costMust.costTypeID eq '%'}">selected="selected"</c:if> >%</option>
							<option value="‰" <c:if test="${costMust.costTypeID eq '‰'}">selected="selected"</c:if>>‰</option>
						</select>
				...
				<div class="form-group ">
					<label class="col-sm-3 control-label no-padding-right" for="form-input"><i class="icon-asterisk orange"></i>应收金额： </label>
					<div class="col-sm-9">
						<input type="text" readonly="readonly" name="mustCostSum" id="mustCostSum" value="${costMust.mustCostSum }" class="col-sm-4 validate[required] getMoney" /> 
						<span class="col-sm-8" style="line-height:26px;">元</span>
					</div>
				</div>
				
				<div class="form-group">
			   	    <label class="col-sm-3 control-label no-padding-right" for="form-input">最迟应收日期： </label>
			        <div class="col-sm-9">
						<div class="input-group col-sm-8">
							<input class="form-control  date-picker mustCostDate validate[custom[date]]" value="<fmt:formatDate value="${costMust.mustCostDate }" pattern="yyyy-MM-dd"/>" type="text" data-date-format="yyyy-mm-dd" name="mustCostDate"/>
				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						<textarea class="col-xs-10 col-sm-8 validate[maxSize[100]]" name="remark"
								rows="5">${costMust.remark }</textarea>
...
<script type="text/javascript">
	...
	$('.costName').attr("value",$('.costTypeID option:selected').text());
