(function ($, z) {
    ...
    function initUpload(param) {
        ...
        $.zjm_singleAttachment.activateUploadBuild(defParam);
    }
                        ...
                        if (data.obj) {
                            $.each(data.obj, function (k, v) {
                                var aHref = ["<div id='" + v.projectFiles_ID + "DIV'><a href='" + v.pathFile + "' target='_blank'>" + v.sourceFileName + "</a>",
                                    '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;',
                                    "<a href='javascript:void(0)' onclick='$.zjm_singleAttachment.deleteOnePictureFile(this.id,this.name)' name='" + v.pathFile + "' id='" + v.projectFiles_ID + "'>" + '删除' + "</a>",
                                    "<br/><div>"].join('');
    ...
    function filesUpdate(project_ID){
		var defParam = {
		...
		var zindex = parseInt($("#loanConfirm").css("z-index"));	//获取第一个模态窗的z-index。bootstrap的默认值，模态窗：z-index=1050，遮罩层：z-index=1040
        ...
        $.zjm_singleAttachment.initUpload(defParam);
