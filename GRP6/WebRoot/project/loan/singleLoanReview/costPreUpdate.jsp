...
<div class="modal fade" id="updateCostPre" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel">修改预收</h4>
      </div>
      <div class="modal-body">
      		<form class="form-horizontal row" role="form" id="updateCostPre_form">
					...
					<div class="col-sm-9">
						...
						<select class="col-sm-4 validate[required] costTypeID" id="costTypeID" name="costTypeID">
							<c:forEach var="costType" items="${costTypeList}" varStatus="status">
								<option <c:if test="${costPre.costTypeID eq costType.dicTypeID}">selected="selected"</c:if> value="${costType.dicTypeID}">${costType.dicTypeName}</option>
							</c:forEach>
						</select>
					</div> 
				...
				<div class="form-group ">
					...
					<div class="col-sm-9">
						<input type="text" value="${costPre.costRate}" name="costRate" class="col-sm-4 validate[required] costRate" onkeyup="calculationFee(this.value);"/> 
					</div>
				</div>
				...
				<div class="form-group ">
					<label class="col-sm-3 control-label no-padding-right" for="form-input"><i class="icon-asterisk orange"></i>预收金额： </label>
					<div class="col-sm-9">
						<input type="text" value="${costPre.preCostSum}" readonly="readonly" name="preCostSum" id="preCostSum" class="col-sm-4 validate[required] getMoney" /> 
						<span class="col-sm-8" style="line-height:26px;">元</span>
					</div>
				</div>
				
	 			<div class="form-group">
			   	    <label class="col-sm-3 control-label no-padding-right" for="form-input"><i class="icon-asterisk orange"></i>预收日期： </label>
			        <div class="col-sm-9">
						<div class="input-group col-sm-8">
							<input value="<fmt:formatDate value="${costPre.preCostDate}" pattern="yyyy-MM-dd"/>" class="form-control  date-picker preCostDate validate[required,custom[date]]" type="text" data-date-format="yyyy-mm-dd" name="preCostDate"/>
				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						<textarea class="col-xs-10 col-sm-8 validate[maxSize[100]]" name="remark"
								rows="5">${costPre.remark}</textarea>
