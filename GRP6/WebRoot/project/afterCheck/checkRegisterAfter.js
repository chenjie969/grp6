(function($,z){
	$.zjm_checkRegister = {
			...
			//autoGenerAfterPlan:autoGenerAfterPlan  //自动生成保后计划
	};
	...
	function initTablePlan(applyID){
		...
		$('#checkPlan-table').bootstrapTable({
			...
			columns: [	
				...
				{title: '检查内容',align: 'center',valign:'middle',row:'3',formatter: function (value, row, index) { return row.checkContent;}},
					...
					formatter : function(value, row, index) {
						...
						if(arr.length > 0){
							...
							$.each(arr,function(k,v){
								...
								"&nbsp;&nbsp;&nbsp;<a href='javascript:void(0)' class='shanchu' title='删除附件' name='"+v.projectFiles_ID+"'>删除</a>"+"<br>"
								*/
								elements="<a target='_blank' style='display:inline' class='dd' id='chakan"+k+"' title='查看附件' name='"+v.pathFile+"' href='javascript:void(0)'>"+v.sourceFileName+"</a> " +
	...
	/*//自动生成保后计划
	function autoGenerAfterPlan(applyID){
		$("#checkSocial_page").load("/project/afterCheck/showAutoGenePlanPage",{},function(response,status,xhr){
			$("#autoGenerTip").modal({keyboard:true});
			tool.undisabled(".btn_submit");
			$(".btn_submit").click(function(){
				tool.disabled(".btn_submit");
				$.ajax({type:'POST',url:'/project/afterCheck/autoGeneAfterPlan',data:{'apply_ID':applyID},contentType:'application/json; charset=UTF-8',dataType:'json',
					success:function(data) {
						if(data.obj){
							$("#autoGenerTip").modal("hide");
							window.location.reload();
						}else{
							alert("自动生成失败！");
							tool.undisabled(".btn_submit");
			...
			$("#autoGenerTip").on("hidden.bs.modal", function (e) {//解除事件绑定
				 $(".btn_submit").unbind("click");
			});
		});
	}*/
...
$(function() {
	...
	var applyID = $('#apply_ID_input').val();
	...
	//自动生成保后计划
	/*$("#btn_autoGenerAfterPlan").click(function() {
		$.zjm_checkRegister.autoGenerAfterPlan(applyID);
	});*/
