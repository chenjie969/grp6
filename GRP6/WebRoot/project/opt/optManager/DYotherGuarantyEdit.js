...
(function($,z){
	 ...
	 * 是否存在第三方
	 */
	function  isHaveOther(isOther,otherType){
		if(isOther == '1'){
			$("#otherOwner").addClass("validate[required,maxSize[50]]"); //给权属人添加验证属性
			if(otherType == '01'){ //法人
				$("#isHaveOtherDIV").show();
				$(".legalPersonInput").addClass("validate[required,maxSize[50]]"); //法人 给法定代表人 等5个字段添加验证属性
				$(".personInput").removeClass("validate[required,maxSize[50]]"); //移除  自然人 验证属性
				$(".person").hide();
				$(".legalPerson").show();
			}else{ // 自然人
				$("#isHaveOtherDIV").show();
				$(".personInput").addClass("validate[required,maxSize[50]]"); //自然人 验证属性添加
				$(".legalPersonInput").removeClass("validate[required,maxSize[50]]"); //法人 删除
				$(".person").show();
				$(".legalPerson").hide();
			}
		}else{
			...
			$(".personInput").removeClass("validate[required,maxSize[50]]"); //移除  自然人 验证属性
			$("#otherOwner").removeClass("validate[required,maxSize[50]]"); //移除权属人验证属性
...
$(function(){
	...
	$(".isOther").change(function(){ 
		...
		$.zjm_DYlandGuaranty.isHaveOther(isOther,otherType);
	})
