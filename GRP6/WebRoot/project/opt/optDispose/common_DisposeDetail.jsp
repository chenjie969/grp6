<%@ page language="java" import="java.util.*" pageEncoding="UTF-8"%>

					<div class="form-group  col-sm-12">
						<h5 class="header smaller lighter blue">保证措施落实详情</h5>
					</div>
					<div class="col-sm-offset-1 col-sm-10">
						<h5 class="col-sm-12">
							抵（质）押登记部门： <span class="grey" id="pledgeDepart">${opt.pledgeDepart }</span>
						</h5>
						<h5 class="col-sm-12">
							<input type="hidden" name="pledgeFile" id="pledgeFile" value="${opt.pledgeFile }">
							抵（质）押登记证明文件： <span class="grey">
							<br><br>
							<div id="pictureDIV_realize" class=""></div>
						</span></h5>
						<h5 class="col-sm-12">
							抵（质）押登记期限： <span class="grey" id="optBeginDate"><fmt:formatDate value="${opt.optBeginDate }" pattern="yyyy-MM-dd"/></span>
										   <span>~</span>
										   <span class="grey" id="optEndDate"><fmt:formatDate value="${opt.optEndDate }" pattern="yyyy-MM-dd"/></span>
						</h5>
						<h5 class="col-sm-12">
							份数： <span class="grey" id="pledgeFileCount">${opt.pledgeFileCount }</span>
						</h5>
						<h5 class="col-sm-12">
							原件存档接收人： <span class="grey" id="receivePerson">${opt.receivePerson }</span>
						</h5>
						<h5 class="col-sm-6">
							登记经办人： <span class="grey" id="realizeUserName">${opt.realizeUserName }</span>
						</h5>
						<h5 class="col-sm-6">
							经办日期： <span class="grey" id="realizeDate"><fmt:formatDate value="${opt.realizeDate }" pattern="yyyy-MM-dd"/></span>
						</h5>
					</div>
					<div class="form-group col-sm-12">
                         <h5 class="header smaller lighter blue">
                         	反担保物处置详情
                         </h5>                             
				  	</div>
				    <div class="col-sm-offset-1 col-sm-10">
						<h5 class="col-sm-12" style="line-height:26px;">
							处置方式： <span class="grey" id="realizeUserName">${opt.disposeTypeName }</span>
						</h5>
						<h5 class="col-sm-12" style="line-height:26px;">
							处置变现金额： <span class="grey"><fmt:formatNumber value="${opt.disposeSum }" pattern="###,###.######"/>万元</span>
						</h5>
						<h5 class="col-sm-12" style="line-height:26px;">
							冲抵变现金额： <span class="grey"><fmt:formatNumber value="${opt.chargeAgainstSum }" pattern="###,###.######"/>万元</span>
						</h5>
						<h5 class="col-sm-12" style="line-height:26px;">
							经办人： <span class="grey" id="realizeUserName">${opt.disposeUserName }</span>
						</h5>
						<h5 class="col-sm-12" style="line-height:26px;">
							处置日期： <span class="grey" id="realizeDate"><fmt:formatDate value="${opt.disposeDate }" pattern="yyyy-MM-dd"/></span>
						</h5>
						<h5 class="col-sm-12" style="line-height:26px;">
							备注： <span class="grey" id="realizeUserName">${opt.guaranteeRemark }</span>
						</h5>	
					</div>
						
						
					