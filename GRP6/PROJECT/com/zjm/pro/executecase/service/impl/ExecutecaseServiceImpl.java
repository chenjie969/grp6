...
public class ExecutecaseServiceImpl implements ExecutecaseService {
	...
	private void conSuitOrAsset(Pro_executecase executecase){
		...
		if (executecase.getLawSuitId() != null && !"".equals(executecase.getLawSuitId())) {
			whereSql = " and projectLawsuit_ID = \'"+ executecase.getLawSuitId() +"\'" ;
		}
		
		//若关联的查封已经关联诉讼，则案件执行也要关联对应的诉讼信息
		if (executecase.getAssetSealId() != null && !"".equals(executecase.getAssetSealId())) {
			whereSql = " and assetSealUp_ID = \'"+ executecase.getAssetSealId() +"\' ";
		}
		...
		if (projectLawsuit != null ){
			...
			executecase.setLawSuitId(projectLawsuit.getProjectLawsuit_ID());
			executecase.setLawSuitName(projectLawsuit.getRecordNum());
		}
	}
