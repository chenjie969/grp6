...
//		mv.getModel().put("applyList", applyList);
    ...
    */
	...
	public ModelAndView viewRiskSchemeInfo(Crm_relationMain crm_relationMain){
		ModelAndView mv = CustomDispatchServlet.getModeAndView();
		Crm_relationMain relationMain = relationMainService.selectOneRelationMainById(crm_relationMain);
		BigDecimal loadSum = projectService.findSumByRelationMainName(relationMain.getRelationMainName());
		
		List<Pro_riskScheme> list= riskSchemeService.selectSchemeByRelaMain(relationMain.getRelationMainName());
		Pro_riskScheme riskScheme =  new Pro_riskScheme();
		List<Pro_projectfiles> fileList = new ArrayList<>();
		riskScheme.setSchemeFileList(fileList);
		List<Pro_suggest> projectSuggestList = new ArrayList<>();
		if(null != list && !list.isEmpty()){	
			for(Pro_riskScheme pro_riskScheme: list){
				String wheresql = " and riskScheme_ID =\'" + pro_riskScheme.getRiskScheme_ID() +"\'";
				String wheresql2 = " and entityID =\'" + pro_riskScheme.getRiskScheme_ID() +"\'";
				
				Pro_riskScheme model = riskSchemeService.selectOneRiskShemeInfo(wheresql);
				if(null != model.getSchemeFileList()){
					riskScheme.getSchemeFileList().addAll(model.getSchemeFileList());
				}
				List<Pro_suggest> modelList = projectSuggestService.selectSuggestListByWhereSql(wheresql2);
				projectSuggestList.addAll(modelList);
			}
			mv.getModelMap().put("riskScheme",riskScheme);
			mv.getModelMap().put("projectSuggestList",projectSuggestList);
			String viewName = "/project/riskResponse/keyProject/riskSchemeWorkInfo";
			mv.setViewName(viewName);
		}
		
		mv.getModel().put("loadSum",loadSum);
		mv.getModel().put("relationMain",relationMain);
		mv.setViewName("/project/riskResponse/keyProject/riskSchemeInfo");
		return mv;
	}
	...
	@ResponseBody
	public AjaxRes findAllCompany(@RequestBody PageTable<Client> pageTable){
		
		List<Client> list = relationMainService.selectAllCompangById(pageTable);
